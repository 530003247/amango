...
class ThinkController extends AdminController {
    ...
    //芒果重写业务   状态必须为STATUS字段
    public function amangostatus($model = null, $id = 0){
        $mod   = M($model);
        $model = M('Model')->find($model);
        $model || $this->error('模型不存在！');
        $sql    = "update ".C('DB_PREFIX').strtolower($model['name'])." set status=(status+1)%2 where id='$id'";
        $res    = $mod->execute($sql);
        if($res!==false ) {
            $this->success('状态操作成功！');
        }else{
    ...
    public function amangosort($model = null, $id = 0){
        $model = M('Model')->find($model);
        $model || $this->error('模型不存在！');
          $mod    = M($model['name']);
