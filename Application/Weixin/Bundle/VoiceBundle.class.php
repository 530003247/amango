...
class VoiceBundle extends Bundle{
    ...
    public function run(){
        global $_W;
        global $_P;
        $keywordinfo = get_posttype_list('voice');
        if(empty($keywordinfo)){
            $new_articles['Title']       = '亲,您发送的语音很不错哦';
            $new_articles['Description'] = 'Sorry,我们暂无处理该图片的方法';
            $new_articles['MusicUrl']    = "";
            $new_articles['HQMusicUrl']  = "";
            $new_articles['ThumbMediaId']= $_W['mediaid'];
            $this->assign('music',$new_articles);
            $this->display();
        } else {
